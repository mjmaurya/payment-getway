...
export const createServerOrder=async(order)=>{
    ...
    try {
        return new Promise((resolve)=>{
            pool.query(`insert into grinzy_dev.payment(order_id,user_order_id,amount) values('${order.order_id}',${order.user_order_id},${100})`,(err,result)=>{
                if (err) {
...
export const updateServerOrder=async(order)=>{
    try {
        return new Promise((resolve)=>{
            pool.query(`update grinzy_dev.payment set payment_id='${order.razorpay_payment_id}' ,payment_status='${order.status}' where order_id='${order.razorpay_order_id}'`,(err,result)=>{
                if (err) {
                    resolve({success:false,...err})
                } else {
                    resolve({success:true,...result})
...
export const serverPaymetByOrderId=async(id)=>{
    try {
        const dbQuery=`SELECT * FROM grinzy_dev.payment where order_id='${id}'`;
        return new Promise((resolve)=>{
            pool.query( dbQuery,
            (err,result)=>{
                if (err) {
